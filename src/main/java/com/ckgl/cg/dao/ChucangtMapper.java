...
public interface ChucangtMapper {
    ...
    @Select("select j.kuanhao,j.yanse,j.ccriqi,j.xs,j.s,j.m,j.l,j.xl,j.xxl,j.xxxl,j.beizhu from chucang_t j order by id desc")
    List<Map> selectAll();
    ...
    @Select("select j.kuanhao,j.yanse,j.ccriqi,j.xs,j.s,j.m,j.l,j.xl,j.xxl,j.xxxl,j.beizhu from chucang_t j where j.kuanhao=#{kuanhao} order by id desc")
