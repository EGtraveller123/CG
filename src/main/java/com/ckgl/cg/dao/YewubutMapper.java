...
public interface YewubutMapper {
    ...
    @Insert("insert into yewubu_t(kuanhao,kehu,yanse,xdriqi,xxs,xs,s,m,l,xl,xxl,xxxl,chriqi,ywshuliang,beizhu) " +
            "values (#{kuanhao},#{kehu},#{yanse},#{xdriqi},#{xxs},#{xs},#{s},#{m},#{l},#{xl},#{xxl},#{xxxl},#{chriqi},#{ywshuliang},#{beizhu})")
    boolean insertYewubut(Yewubut yewubut);

    @Select("select id,kuanhao,kehu,yanse,xxs,xs,s,m,l,xl,xxl,xxxl,chriqi,xdriqi,ywshuliang,beizhu from yewubu_t where kehu=#{kehu} order by ${sortName} ${sortOrder}")
    List<Map> selectByKehu(@Param("kehu") String kehu,@Param("sortName") String sortName,@Param("sortOrder") String sortOrder);

    @Select("select id,kuanhao,kehu,yanse,xxs,xs,s,m,l,xl,xxl,xxxl,chriqi,xdriqi,ywshuliang,beizhu from yewubu_t where kuanhao=#{kuanhao} order by ${sortName} ${sortOrder}")
